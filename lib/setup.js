...
const createDestinationDirectory = fileDestination => {
    const fileDestinationDirectories = fileDestination.split('/');

    // Remove the last element from the array, being the destination file itself.
    fileDestinationDirectories.pop();
    if (fileDestinationDirectories.length < 1) {
        return;
    }

    let destinationDirectory = '.';
    for (const directory of fileDestinationDirectories) {
        destinationDirectory += `/${directory}`;
        const directoryExists = fileSystem.existsSync(destinationDirectory);
        if (!directoryExists) {
            fileSystem.mkdirSync(destinationDirectory);
        }
    }
};
...
const moveFiles = filesToCopy => {
    filesToCopy.forEach((fileToCopy) => {
        createDestinationDirectory(fileToCopy.fileDestination);
...
const setupCodeStyleElement = codeStyleFilePath => {
    ...
    if (codeStyle.filesToCopy) {
        moveFiles(codeStyle.filesToCopy);
    }
    return codeStyle;
};

const retrieveCodeStylesElements = (codeStyle) => new Promise(resolve => {
    fileSystem.readdir(
...
const runSetup = async (version) => {
    ...
    const questions = [
        {
            type: 'checkbox',
            name: 'codeTypes',
            message: 'Please select the code style setups your project will use',
            choices: codeStyles
        },
    ];
    ...
    const codeStylePromises = await userResponse['codeTypes'].map(async codeStyle => {
